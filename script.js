...
const form=$('#projectForm'),status=$('#formStatus');
form?.addEventListener('submit',async e=>{
 ...
 if(oversized.length){if(status)status.textContent='Please remove files larger than 20 MB before sending.';return}
 if(totalSize>20*1024*1024){if(status)status.textContent='Please keep the combined attachment size below 20 MB.';return}
 if(!window.mabcorSupabase){if(status)status.textContent='The enquiry service is temporarily unavailable. Please use WhatsApp or email us directly.';return}
 ...
 try{
   ...
   if(status)status.textContent=`✓ Enquiry received${result.reference?` · Reference ${result.reference}`:''}. Thank you — Mabcor will contact you.`;
 }catch(error){console.error(error);if(status)status.textContent='We could not send your enquiry right now. Please try again or use the WhatsApp button.'}
