...
function buildHero(){
 ...
 wrap.innerHTML=''; hero.forEach((src,i)=>{const d=document.createElement('div');d.className='hero-slide'+(i===0?' active':'');d.dataset.src=src;if(i<2)d.style.backgroundImage=`url('${src}')`;wrap.appendChild(d)});
...
$$('[data-cap-image]').forEach(img=>{const src=DATA.capabilities?.[img.dataset.capImage];if(src)img.src=src});
...
$$('.media-slideshow').forEach((el,si)=>{
 ...
 el.innerHTML='';items.forEach((src,i)=>{const im=document.createElement('img');im.src=src;im.alt='';im.loading=i?'lazy':'eager';im.className=i===0?'active':'';el.appendChild(im)});
...
$$('[data-bg-slot]').forEach((el,n)=>{const list=DATA.slots?.[el.dataset.bgSlot]||[];if(!list.length)return;let i=0;el.style.backgroundImage=`url('${list[0]}')`;setInterval(()=>{i=(i+1)%list.length;el.style.backgroundImage=`url('${list[i]}')`},7200+n*450)});

// Static editable Start a Project background image
const contactSection=$('[data-contact-background]');
if(contactSection){const src=DATA.slots?.['contact-image']?.[0]||window.MABCOR_DEFAULT_DATA?.slots?.['contact-image']?.[0];if(src)contactSection.style.backgroundImage=`url('${src}')`;}

// News / community section
const newsGrid=$('#newsGrid');if(newsGrid){newsGrid.innerHTML=(DATA.news||[]).map((n,i)=>`<article class="news-card"><figure><img src="${n.image||''}" alt="${n.title||'Mabcor news'}"></figure><div class="news-copy"><span>${n.category||'NEWS'}</span><h3>${n.title||'Mabcor update'}</h3><p>${n.text||''}</p></div></article>`).join('')}
...
const revealTargets=$$('.company-grid,.section-head,.cap-grid,.logistics-copy,.project-gallery,.plant-intro,.plant-gallery,.delivery-grid,.credential-grid,.news-grid,.contact-copy,.project-form');revealTargets.forEach(el=>el.classList.add('reveal-ready'));const observer=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('revealed');observer.unobserve(e.target)}}),{threshold:.08});revealTargets.forEach(el=>observer.observe(el));
