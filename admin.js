...
const targetDefs=[
  ...
  ['slot:electrical-feature-thumb','Electrical showcase — Thumbnail slideshow',5],['slot:electrical-feature-bg','Electrical showcase — Background slideshow',10],['slot:logistics-main','Logistics feature slideshow',5],['slot:project-civil','Projects — Civil slideshow',5],['slot:project-site','Projects — Active site slideshow',5],['slot:project-electrical','Projects — Electrical slideshow',5],['slot:project-logistics','Projects — Logistics slideshow',5],['slot:plant-main','Plant — Main slideshow',5],['slot:plant-support','Plant — Support slideshow',5],['slot:plant-fleet','Plant — Fleet slideshow',5],['slot:parallax-civil','Parallax — Infrastructure',5],['slot:parallax-logistics','Parallax — Logistics',5],['slot:contact-image','Start a Project — Background image',1]
];
...
async function loadContent(){
  ...
  ['electrical-feature-thumb','electrical-feature-bg'].forEach(key=>{
    if(!Array.isArray(data.slots[key]) || !data.slots[key].length){
      data.slots[key]=[...(window.MABCOR_DEFAULT_DATA?.slots?.[key]||[])];
    }
  });
  dirty=false;
}
...
function renderContact(){const c=data.contact||{};$('#phone').value=c.phone||'';$('#phoneHref').value=c.phoneHref||'';$('#email').value=c.email||'';$('#location').value=c.location||'';$('#whatsapp').value=c.whatsapp||'';$('#notificationEmail').value=c.notificationEmail||'msindisi.mtengwane@gmail.com'}
function bindContact(){['phone','phoneHref','email','location','whatsapp','notificationEmail'].forEach(key=>$('#'+key).addEventListener('input',()=>{data.contact ||= {};data.contact[key]=$('#'+key).value;markDirty()}))}
function formatDate(value){try{return new Intl.DateTimeFormat('en-ZA',{dateStyle:'medium',timeStyle:'short'}).format(new Date(value))}catch(_){return value||''}}
function renderEnquirySummary(){const box=$('#enquirySummary');if(!box)return;const counts={New:0,'In Progress':0,Closed:0};enquiries.forEach(e=>counts[e.status||'New']=(counts[e.status||'New']||0)+1);box.innerHTML=Object.entries(counts).map(([k,v])=>`<div class="enquiry-stat"><strong>${v}</strong><span>${esc(k)}</span></div>`).join('');const badge=$('#enquiryBadge');if(badge){const n=counts.New||0;badge.textContent=n?String(n):'';badge.className=n?'enquiry-badge':''}}
async function openAttachment(path,name){try{const {data:url,error}=await db.storage.from('enquiry-attachments').createSignedUrl(path,120);if(error)throw error;window.open(url.signedUrl,'_blank','noopener')}catch(error){setStatus(`Could not open ${name||'attachment'}: ${error.message}`,'bad')}}
function renderEnquiries(){renderEnquirySummary();const box=$('#enquiryList');if(!box)return;box.innerHTML='';if(!enquiries.length){box.innerHTML='<div class="empty">No website enquiries have been received yet.</div>';return}enquiries.forEach(item=>{const card=document.createElement('article');card.className='enquiry-card';const attachments=Array.isArray(item.attachments)?item.attachments:[];card.innerHTML=`<div><div class="enquiry-type">${esc(item.enquiry_type||'General enquiry')}</div><h3>${esc(item.name||'Website enquiry')}${item.company?` · ${esc(item.company)}`:''}</h3><div class="enquiry-meta"><span>${esc(formatDate(item.created_at))}</span>${item.location?`<span>${esc(item.location)}</span>`:''}${item.email?`<span>${esc(item.email)}</span>`:''}${item.phone?`<span>${esc(item.phone)}</span>`:''}</div><p class="enquiry-message">${esc(item.message||'')}</p><div class="enquiry-links">${item.email?`<a href="mailto:${esc(item.email)}">Email sender</a>`:''}${item.phone?`<a href="tel:${esc(item.phone)}">Call</a>`:''}${attachments.map((a,i)=>`<button type="button" data-att="${i}">${esc(a.name||`Attachment ${i+1}`)}</button>`).join('')}</div></div><div class="enquiry-actions"><label><span class="hint">STATUS</span><select><option>New</option><option>In Progress</option><option>Closed</option></select></label><span class="hint">${attachments.length} attachment${attachments.length===1?'':'s'}</span></div>`;const sel=card.querySelector('select');sel.value=item.status||'New';sel.addEventListener('change',async()=>{const next=sel.value;sel.disabled=true;try{const {error}=await db.from('website_enquiries').update({status:next}).eq('id',item.id);if(error)throw error;item.status=next;renderEnquirySummary();setStatus('Enquiry status updated.','good')}catch(error){sel.value=item.status||'New';setStatus(error.message,'bad')}finally{sel.disabled=false}});card.querySelectorAll('[data-att]').forEach(btn=>btn.addEventListener('click',()=>{const a=attachments[Number(btn.dataset.att)];if(a?.path)openAttachment(a.path,a.name)}));box.appendChild(card)})}
async function loadEnquiries(){const box=$('#enquiryList');if(box)box.innerHTML='<div class="empty">Loading enquiries…</div>';try{const {data:rows,error}=await db.from('website_enquiries').select('*').order('created_at',{ascending:false}).limit(200);if(error)throw error;enquiries=rows||[];enquiriesLoaded=true;renderEnquiries()}catch(error){if(box)box.innerHTML=`<div class="empty">Could not load enquiries: ${esc(error.message)}</div>`;setStatus(`Could not load enquiries: ${error.message}`,'bad')}}
...
$$('.tab').forEach(btn=>btn.addEventListener('click',()=>{$$('.tab,.panel').forEach(e=>e.classList.remove('active'));btn.classList.add('active');$(`#panel-${btn.dataset.tab}`).classList.add('active');if(btn.dataset.tab==='enquiries'&&!enquiriesLoaded)loadEnquiries()}));
$('#heroSearch').addEventListener('input',renderHero);$('#imageSearch').addEventListener('input',renderTarget);bindContact();$('#refreshEnquiriesBtn')?.addEventListener('click',loadEnquiries);
