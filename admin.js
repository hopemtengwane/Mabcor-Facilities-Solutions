...
function mediaItem(m,fn){const d=document.createElement('div');d.className='media';d.innerHTML=`<img loading="lazy" src="${esc(resolveMediaUrl(m.src))}" alt=""><span title="${esc(m.original||m.src)}">${esc(m.original||m.src)}</span>`;d.onclick=()=>fn(m);return d}
