...
async function loadUploadedMedia(){
  try{
    ...
    media=uniqueMedia([...(window.MABCOR_MEDIA||[]),...uploaded]);
  }catch(error){console.warn('Could not list uploaded media.',error);media=uniqueMedia([...(window.MABCOR_MEDIA||[])]);}
}
...
async function uploadImage(file){
  ...
  const path=`uploads/${Date.now()}-${clean}.${ext}`;
  ...
  const item={id:`supabase-${Date.now()}`,src:url.publicUrl,original:file.name,remote:true};
  media=uniqueMedia([...media,item]);
  return item;
}

function mediaItem(m,fn){const d=document.createElement('div');d.className='media';d.innerHTML=`<img loading="lazy" src="${esc(m.src)}" alt=""><span title="${esc(m.original||m.src)}">${esc(m.original||m.src)}</span>`;d.onclick=()=>fn(m);return d}
...
function renderTarget(){
  ...
  t.list.forEach((src,i)=>{const d=document.createElement('div');d.className='item';d.innerHTML=`<img src="${esc(src)}" alt=""><div class="meta">${i+1} of ${t.max}</div><div class="item-buttons"><button data-a="up" title="Move left">←</button><button data-a="down" title="Move right">→</button><button data-a="remove" title="Remove">×</button></div>`;d.querySelector('[data-a="remove"]').onclick=()=>{const list=getTarget().list;list.splice(i,1);setTarget(list);renderTarget()};d.querySelector('[data-a="up"]').onclick=()=>{const list=getTarget().list;move(list,i,-1);setTarget(list);renderTarget()};d.querySelector('[data-a="down"]').onclick=()=>{const list=getTarget().list;move(list,i,1);setTarget(list);renderTarget()};box.appendChild(d)});
...
function renderNews(){
  ...
  (data.news||[]).forEach((item,i)=>{
    ...
    card.innerHTML=`<img class="news-preview" src="${esc(item.image||'')}" alt=""><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><h3>Article ${i+1}</h3><button class="btn ghost news-delete" type="button">Delete</button></div><label><span>Category</span><input data-k="category" value="${esc(item.category||'NEWS')}"></label><label><span>Headline</span><input data-k="title" value="${esc(item.title||'')}"></label><label><span>Article text</span><textarea data-k="text">${esc(item.text||'')}</textarea></label><div class="upload" style="margin:2px 0 0;padding:14px"><input class="news-image-input" type="file" accept="image/jpeg,image/png,image/webp"><button class="btn ghost news-image-upload" type="button">Upload / replace image</button></div>`;
    ...
    card.querySelector('.news-image-upload').onclick=async()=>{
      const input=card.querySelector('.news-image-input'),file=input.files[0];if(!file){setStatus('Choose an image for this article first.','bad');return}
      const btn=card.querySelector('.news-image-upload');btn.disabled=true;btn.textContent='Uploading…';
      try{const uploaded=await uploadImage(file);data.news[i].image=uploaded.src;markDirty();renderNews();renderHero();renderTarget();setStatus(`${uploaded.original} uploaded and assigned to Article ${i+1}. Click Save Changes to publish.`,'good')}catch(error){setStatus(error.message,'bad')}finally{btn.disabled=false}
    };
...
$('#uploadBtn').addEventListener('click',async()=>{const file=$('#uploadInput').files[0];if(!file){setStatus('Choose an image before uploading.','bad');return}const btn=$('#uploadBtn');btn.disabled=true;btn.textContent='Uploading…';try{const item=await uploadImage(file);$('#uploadInput').value='';renderHero();renderTarget();setStatus(`${item.original} uploaded to Supabase Storage. It is now available in the media library.`,'good')}catch(error){setStatus(error.message,'bad')}finally{btn.disabled=false;btn.textContent='Upload to library'}});

$('#targetUploadBtn').addEventListener('click',async()=>{
  const file=$('#targetUploadInput').files[0];
  if(!file){setStatus('Choose an image before uploading.','bad');return}
  const t=getTarget();
  if(t.max>1 && t.list.length>=t.max){setStatus(`This slideshow already has ${t.max} images. Remove one first, then upload the new image.`,'bad');return}
  ...
  try{
    ...
    if(t.max===1) list=[item.src];
    else if(!list.includes(item.src)) list.push(item.src);
    setTarget(list);
    $('#targetUploadInput').value='';
    renderHero();renderTarget();
    const label=targetDefs.find(d=>d[0]===currentTarget)?.[1]||'selected section';
    setStatus(`${item.original} uploaded and added to ${label}. Click Save Changes to publish it.`,'good');
  }catch(error){setStatus(error.message,'bad')}
  finally{btn.disabled=false;btn.textContent='Upload & add here'}
});
